...
function loadFiles({
  cwd = process.cwd(),
  ignore = ['**/__tests__/**', '**/test/**', '**/backend.js', '**/setupTests.js', '**/setupProxy.js', '**/*.d.ts'],
  ...rest
} = {}) {
  const fileInfo = glob.sync('src/**/*.+(js|html|jsx|ts|tsx|md|mdx)', {
    cwd,
    ignore,
    ...rest
  ...
  .map(filePath => {
    ...
    if (parentDir === 'final' || parentDir === 'exercise') {
      if (ext === '.js' || ext === '.tsx' || ext === '.ts') {
        ...
        const extraCreditTitleMatch = (secondLine == null ? void 0 : secondLine.match(/\/\/ 💯 (?<title>.*)$/)) || fallbackMatch;
